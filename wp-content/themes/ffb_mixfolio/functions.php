...
function mixfolio_setup() {
	...
	set_post_thumbnail_size( 600, 600, true ); // 600 pixels wide by 600 pixels high, hard crop mode
	add_image_size( 'mixfolio-featured-thumbnail', 600, 600, true ); // 600 pixels wide by 600 pixels high, hard crop mode */
	add_image_size( 'map-thumbnail', 300, 300, true );
	/**
...
function mixfolio_scripts() {
	...
	if (
		...
		wp_register_script( 'tweetable', get_template_directory_uri() . '/js/jquery.tweetable.js', array( 'jquery' ), '1.6' );
...
add_action( 'load-post.php', 'gip_email_setup' );
add_action( 'load-post-new.php', 'gip_email_setup' );

/* Meta box setup function. */
function gip_email_setup() {

	/* Add meta boxes on the 'add_meta_boxes' hook. */
	add_action( 'add_meta_boxes', 'gip_email_add_meta_boxes' );
}

/* add metabox to backend */

function gip_email_add_meta_boxes() {
	add_meta_box(
		'gip_email',			// Unique ID
		esc_html__( 'User email', 'example' ),		// Title
		'gip_email_meta_box',		// Callback function
...
/* Display the post email meta box */

function gip_email_meta_box( $object, $box ) { ?>

	<?php wp_nonce_field( basename( __FILE__ ), 'gip_email_nonce' ); ?>

	<p>
		<label for="gip_email"><?php _e( "User email", '...' ); ?></label>
		<br />
		<input class="widefat" type="text" name="gip_email" id="gip_email" value="<?php echo esc_attr( get_post_meta( $object->ID, 'gip_email', true ) ); ?>" size="30" />
	</p>
<?php }
