...
function colabs_image($args) {
	...
	//Defaults
	if (get_option($shortname.'_custom_field_image')!=''){
		$key = get_option($shortname.'_custom_field_image');
	}else{
		$key = 'image';
	}
	$width = null;
	...
	} elseif ( 'true' == get_option( $shortname.'_post_image_support') AND !empty($thumb_id) ) {

		if ( get_option( $shortname.'_pis_resize') == "true") {
		
			// Dynamically resize the post thumbnail 
			$vt_crop = get_option( $shortname.'_pis_hard_crop' );
	...
	if ( empty($custom_field) && 'true' == get_option( $shortname.'_auto_img') && empty($img_link) && !(is_singular() AND in_the_loop() AND $link == "src") ) { 
		...
		if ( !empty($attachments) ) {  
			...
			foreach ( $attachments as $att_id => $attachment ) {            
				...
				if ( 'true' == get_option($shortname.'_post_image_support') AND get_option( $shortname.'_pis_resize') == "true") {
	...
	if(!empty($img_link) ){
		...
		foreach($src_arr as $key => $custom_field){
			...
			if ( 'img' == $link ) {  // Just output the image 
				...
				if ( isset($vt_image['url']) || $is_auto_image ) { 
...
// Depreciated
function colabs_get_embed($key = '', $width, $height, $class = 'video', $id = null) {
	$shortname = get_option( 'colabs_shortname' );
	$key = $shortname.'_embed';
	// Run new function
...
function colabs_show_pagemenu( $exclude="" ) {
    ...
    if ( get_option( $shortname.'_ex_featpages') ) {
        $menupages = get_option( $shortname.'_featpages' );
...
function colabs_style_path() {
	...
	if ( $style != '' ) {
		...
		$stylesheet = get_option( $shortname.'_alt_stylesheet' );
...
/*-----------------------------------------------------------------------------------*/
$shortname = get_option( 'colabs_shortname' );
if (get_option($shortname.'_rss_thumb') == "true"){
...
function colabs_gosquared(){
	$shortname = get_option( 'colabs_shortname' );
	$output = get_option( $shortname.'_gosquared_id' );
	if ( $output <> "" ){
?>
<!-- GoSquared Analytics -->
...
if ( !function_exists( 'colabs_twitter_script') ) {
    ...
    <?php
		...
		wp_enqueue_script( 'twitterjs', 'http://api.twitter.com/1/statuses/user_timeline/'. $username.'.json?callback=twitterCallback2&amp;count='.$limit.'&amp;include_rts=t'.$exclude_replies_str, array( 'jquery' ));
...
function colabs_short_url($url) {
	$shortname = get_option( 'colabs_shortname' );
	$service = get_option( $shortname.'_url_shorten' );
	$bitlyapilogin = get_option( $shortname.'_bitly_api_login' );
	$bitlyapikey = get_option( $shortname.'_bitly_api_key' );
	if (isset($service)) {
		switch ($service) 
		{
    		case 'TinyURL':
	...
	function colabs_google_webfonts() {
		...
		if ( !empty($colabs_options) ) {
			...
			if ( $fonts ) {
				...
				wp_enqueue_style( 'googlefont', 'http'. ( is_ssl() ? 's' : '' ) .'://fonts.googleapis.com/css?family=' . $fonts);
...
if ( !function_exists( 'colabsthemes_more_themes_page') ) {
	function colabsthemes_more_themes_page(){
        ...
        <div class="wrap themes-page">
	        <h2><?php _e('More ColorLabs Themes','colabthemes');?></h2>
	        ...
	        ?>
	        <div class="info">
		        <a href="http://colorlabsproject.com/member/member.php"><?php _e('Member Area','colabthemes');?></a>
		        <a href="http://colorlabsproject.com/themes/"><?php _e('Themes Gallery','colabthemes');?></a>
		        <a href="http://colorlabsproject.com/showcase/"><?php _e('Theme Showcase','colabthemes');?></a>
	        </div>
...
if ( ! function_exists( 'colabs_pagination' ) ) {
	...
	function colabs_pagination( $args = array(), $query = '' ) {
		...
		if( $wp_rewrite->using_permalinks() && isset($raw_querystring[0]) ){
			$rawquery = str_replace( '', '', $raw_querystring[0] );
			$args['base'] = str_replace( $rawquery, '', $args['base'] );
			$args['base'] .= substr( $rawquery, 0, -1 );
		}
...
function colabs_get_dynamic_values ( $settings ) {
	...
	if ( is_array( $colabs_options ) ) {
		foreach ( $settings as $k => $v ) {
			if ( isset( $colabs_options[$shortname.'_' . $k] ) && ( $colabs_options[$shortname.'_' . $k] != '' ) ) { $settings[$k] = $colabs_options[$shortname.'_' . $k]; }
		}
	}
...
function og_meta(){ 
	$shortname = get_option( 'colabs_shortname' );
	?>
	<?php if ( is_home() && '' == get_option( $shortname.'_og_enable' ) ) { ?>
	<meta property="og:title" content="<?php echo bloginfo('name');; ?>" />
	<meta property="og:type" content="author" />
	<meta property="og:url" content="<?php home_url(); ?>" />
	<meta property="og:image" content="<?php echo get_option($shortname.'_og_img'); ?>"/>
	<meta property="og:site_name" content="<?php echo get_option($shortname.'_og_sitename'); ?>" />
	<meta property="fb:admins" content="<?php echo get_option($shortname.'_og_admins'); ?>" />
	<meta property="og:description" content="<?php echo get_option('blogdescription '); ?>" />
	<?php } ?>
	
	<?php if ( ( is_page() || is_single() ) && '' == get_option( $shortname.'_og_enable' ) ) { ?>
...
global $colabs_options;
	$shortname = get_option( 'colabs_shortname' );
	if ( 'true' == $colabs_options[ $shortname.'_pt_enable' ] ) {
...
$shortname = get_option( 'colabs_shortname' );
if( $colabs_options[ $shortname.'_pt_enable' ] != 'true' ) { return; }
...
if ( ! function_exists( 'colabs_feed_link' ) ) { 
	function colabs_feed_link ( $output, $feed = null ) {
		...
		if ( isset($colabs_options[ $shortname.'_feedlinkurl' ]) && ( $feed == $default ) && ( ! stristr( $output, 'comments' ) ) ) $output = $colabs_options[ $shortname.'_feedlinkurl' ];	
		return $output;	
...
if ( ! function_exists( 'colabs_custom_comments_rss' ) ) { 
	function colabs_custom_comments_rss( $post_id = '', $feed = '',$text = '' ) {
		...
		if ( $colabs_options[ $shortname.'_feedlinkcomments' ] !='') $url= $colabs_options[ $shortname.'_feedlinkcomments' ];
