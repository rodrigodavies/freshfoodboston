...
class CoLabsThemes_Backup {
	...
	function register_admin_screen () {
		...
		$this->admin_page = add_theme_page(__( 'ColorLabs Import/Export Settings', 'colabsthemes' ), __( 'Import/Export Settings', 'colabsthemes' ), 'manage_options', $this->token, array( &$this, 'admin_screen' ) );
    ...
    function register_admin_head(){
        ...
        wp_enqueue_style( 'adminstyle', get_template_directory_uri() . '/functions/admin-style.css');
...
<div id="colabs_options" class="wrap <?php if (is_rtl()) { echo 'rtl'; } ?> colabs_backup">
    ...
    <div class="section">
    	...
    	<div class="form-wrap">
    		<form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
    			...
    			<input type="submit" class="button" value="<?php esc_attr_e( 'Upload File and Import', 'colabsthemes' ); ?>" />
    		</form>
    	</div><!--/.form-wrap-->
    	</div><!-- .option -->
    ...
    <div class="section">
    	...
    	<form method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
    		...
    		<input type="submit" class="button" value="<?php esc_attr_e( 'Download Export File', 'colabsthemes' ); ?>" />
    	</form>
    	</div><!-- .option -->
...
<?php
	...
	function move_admin_menu () {
		...
		foreach ( $submenu['colabsthemes'] as $k => $s ) {
			...
			if ( in_array( $s[2], array( 'colabsthemes_themes' ) ) ) {
				$below_items[] = $s;
				unset( $submenu['colabsthemes'][$k] );
			}
