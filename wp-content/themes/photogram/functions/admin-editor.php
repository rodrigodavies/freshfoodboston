...
class colabs_custom_editor {
	...
	function is_custom_writable($file, $files) {
        ...
        elseif( $_GET['updated']=='false') // Update Failed
	...
	<div class="section">
		...
		<div class="option">
			<form method="post" enctype="multipart/form-data" id="colabsform" action="<?php echo admin_url('admin-post.php?action=colabs_file_editor'); ?>">
				...
				<input type="submit" class="button" value="<?php esc_attr_e( 'Upload File', 'colabsthemes' ); ?>" name="custom_file_import" />
			</form>
		</div><!-- .option -->
	</div><!-- .section -->    
    ...
    <div class="option">
        <form style="overflow:hidden" method="post" id="file-jump" name="file-jump" action="<?php echo admin_url('admin-post.php?action=colabs_file_editor'); ?>">
        	...
        	<p>
        		...
        		<input type="submit" id="custom_file_jump" name="custom_file_jump" value="<?php esc_attr_e('Edit selected file', 'colabsthemes'); ?>" />
        	</p>
        ...
        <form class="file_editor" method="post" id="template" name="template" action="<?php echo admin_url('admin-post.php?action=colabs_file_editor'); ?>">
            <div class="save_bar_top right">
                ...
                <?php wp_nonce_field('colabs-custom-file', '_wpnonce-colabs-custom-file'); ?>
                <input type="submit" value="<?php esc_attr_e( 'Save All Changes', 'colabsthemes' ); ?>" class="button submit-button button-primary" name="custom_file_submit"/>
            </div>        
        	...
        	<span>
        		<?php wp_nonce_field('colabs-custom-file', '_wpnonce-colabs-custom-file'); ?>
          <input type="submit" class="button submit-button button-primary save_button" id="custom_file_submit" value="<?php esc_attr_e( 'Save All Changes', 'colabsthemes' ); ?>" name="custom_file_submit" />
...
function colabs_load_only_editor(){
    ...
    function colabs_admin_head_editor(){
		...
		wp_enqueue_style( 'colorpicker', get_template_directory_uri() . '/functions/css/colorpicker.css');
		wp_enqueue_style( 'codemirror', get_template_directory_uri() . '/functions/css/codemirror.css');  
		wp_enqueue_script( 'colorpickerjs', get_template_directory_uri() . '/functions/js/colorpicker.js', array('jquery') );
		wp_enqueue_script( 'codemirrorjs', get_template_directory_uri() . '/functions/js/codemirror.js', array('jquery') );	
?>
        <style type="text/css">
            ...
            .colabs_container .save_bar_top{ position: absolute; right: 18px; top: -35px }
      ...
      var cssEditor = CodeMirror.fromTextArea(document.getElementById("newcontent"), {
        ...
        onCursorActivity: function() {
          cssEditor.setLineClass(hlLine, null, null);
          hlLine = cssEditor.setLineClass(cssEditor.getCursor().line, null, "activeline");
        },
        onChange: function() {
