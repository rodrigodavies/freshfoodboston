...
if (!function_exists( 'colabs_output_alt_stylesheet')) {
	function colabs_output_alt_stylesheet() {
		...
		echo "<!-- Alt Stylesheet -->\n";
		if ($style != '') {
			wp_enqueue_style( 'skinstyle', get_template_directory_uri() .'/styles/'. $style);
			...
			if( $style != '' ) {
				...
				wp_enqueue_style( 'skinstyle', get_template_directory_uri() .'/styles/default.css');
...
if (!function_exists( 'colabs_head_css')) {
	function colabs_head_css() {
		...
		$output = '';
		$text_title = get_option( $shortname.'_texttitle' );
		$tagline = get_option( $shortname.'_tagline' );
...
if (!function_exists( 'colabs_output_custom_css')) {
	function colabs_output_custom_css() {
		// Custom.css insert
		wp_enqueue_style( 'customstyle', get_template_directory_uri() .'/custom/custom.css');
...
if(function_exists( 'add_theme_support')){
	$shortname = get_option( 'colabs_shortname' );
	if('true' == get_option( $shortname.'_post_image_support')){
		add_theme_support( 'post-thumbnails' );
		// set height, width and crop if dynamic resize functionality isn't enabled
		if ( get_option( $shortname.'_pis_resize') <> "true" ) {
			$thumb_width = get_option( $shortname.'_thumb_w' );
			$thumb_height = get_option( $shortname.'_thumb_h' );
			$single_width = get_option( $shortname.'_single_w' );
			$single_height = get_option( $shortname.'_single_h' );
			$hard_crop = get_option( $shortname.'_pis_hard_crop' );
